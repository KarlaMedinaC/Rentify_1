...
<body>
    ...
    <section id="propiedades" class="propiedades">
        <h1 class="p-3 mb-2 bg-dark text-white">Propiedades Pendientes</h1>
        

        <div class="border p-5">
        <div class="form-group">
            <a href="propiedades.php" class="btn btn-primary">Propiedades aprobadas</a>
        </div>
            <table class="table table-striped">
                <thead>
                    <tr>
                        ...
                        <th scope="col">Comprobar status</th>
                    </tr>
                </thead>
                <tbody>
                    <?php
                    ...
                    $consulta_propiedades = "SELECT propietario.nombre AS Propietario, propiedad.id_publicacionpropiedad AS ide, propiedad.operacion AS Operación, propiedad.inmueble AS Inmueble, propiedad.direccion AS Dirección, propiedad.recamaras AS Recámaras, propiedad.banos AS Baños, propiedad.estacionamiento AS Estacionamiento, propiedad.m2construccion AS 'M2 Construcción', propiedad.m2terreno AS 'M2 Terreno', propiedad.niveles AS Niveles, propiedad.precio AS Precio, propiedad.servicios AS Servicios, propiedad.descripcion AS Descripción
                                        FROM propiedad
                                        INNER JOIN propietario ON propiedad.id_propietario = propietario.id_propietario 
                                        WHERE propiedad.status = 'Pendiente' or propiedad.status = ''";
                    ...
                    if (!$ejecutarConsultaPropiedades) {
                        ...
                        if ($verFilasPropiedades < 1) {
                            echo "<tr><td colspan='13'>Sin propiedades</td></tr>";
                        } else {
                            while ($filaPropiedad = mysqli_fetch_array($ejecutarConsultaPropiedades)) {
                                echo '<tr>
                                    <td>' . $filaPropiedad['Propietario'] . '</td>
                                    <td>' . $filaPropiedad['Operación'] . '</td>
                                    <td>' . $filaPropiedad['Inmueble'] . '</td>
                                    <td>' . $filaPropiedad['Dirección'] . '</td>
                                    <td>' . $filaPropiedad['Servicios'] . '</td>
                                    <td>' . $filaPropiedad['Descripción'] . '</td>
                                    <td><a href="revisarStatus.php?id='.$filaPropiedad['ide'].'">Revisar</a></td>
                                </tr>';
                            }
                        }
                    }
