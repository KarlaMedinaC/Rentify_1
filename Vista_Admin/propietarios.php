...
<body>
    ...
    <h1 class="p-3 mb-2 bg-dark text-white">Propietarios</h1>
        ...
        <table class="table table-striped">
            <thead>
                <tr>
                    ...
                    <th scope="col">Mostrar propiedades</th>
                </tr>
            </thead>
            <tbody>
                <?php
                ...
                if (isset($_GET['buscarNombre'])) {
                    ...
                    $consulta = "SELECT * FROM propietario WHERE nombre LIKE '%$nombre%'";
                } else {
                ...
                if (!$ejecutarConsulta) {
                    ...
                    if ($verFilas < 1) {
                        echo "<tr><td colspan='11'>Sin registros</td></tr>";
                    } else {
                        while ($fila = mysqli_fetch_array($ejecutarConsulta)) {
                            ...
                            echo '<tr>
                                    ...
                                    <td>'.$fila[6].'</td>
                                    <td>'.$fila[7].'</td>
                                    <td><a href="tabla_propiedades.php?id='.$id_propietario.'">Motrar Registros ['.$verFilasPropiedades.']</a></td>
...
function eliminarPropietario(id) {
    if (confirm("¿Estás seguro de que deseas eliminar este propietario?")) {
        // Realizar una petición AJAX para eliminar el propietario
        var xhttp = new XMLHttpRequest();
        xhttp.onreadystatechange = function() {
            if (this.readyState == 4 && this.status == 200) {
                // Actualizar la página o realizar alguna acción adicional después de eliminar el propietario
                location.reload(); // Actualizar la página para reflejar los cambios
            }
        };
        xhttp.open("GET", "../EliminarRegistros/eliminar_propietario.php?id=" + id, true);
        xhttp.send();
    }
}
